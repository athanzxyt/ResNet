...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, num_blocks, out_channels, stride):
        ...
        if stride != 1 or self.in_channels != out_channels*block.expansion:
            identity_shortcut = nn.Sequential(
...
ResNet models
"""
def ResNet18(channels, num_classes):
    return ResNet(block_2layer, [2,2,2,2], channels, num_classes)

def ResNet34(channels, num_classes):
...
def ResNet101(channels, num_classes):
    return ResNet(block_3layer, [3,4,23,3], channels, num_classes)

def ResNet152(channels, num_classes):
